...
var genreCheckN = [], genreCheckI = [], genreCheckA = [], genreCheckAd = [], genreCheckC = [],
genreCheckS = [], genreCheckR = [], genreCheckSi = [], genreCheckE = [], genreCheckSp = [],
genreCheckRa = [], genreCheckM = [];

var genreNf = [], genreIf = [], genreAf = [], genreAdf = [], genreCf = [], genreSf= [],
genreRf = [], genreSif = [], genreEf = [], genreSpf = [], genreRaf = [], genreMf = [];

var genreNp = [], genreIp = [], genreAp = [], genreAdp = [], genreCp = [], genreSp = [],
genreRp = [], genreSip = [], genreEp = [], genreSpp = [], genreRap = [], genreMp = [];

//images
var logo, img;

//change graph
var sceneSwitchF = false; sceneSwitchP = false, sceneSwitchT = false;
...
function setup() {
  ...
  for (var i = 0; i < dataTable.getRowCount(); i++) {
    ...
    genreCheckN[i] = dataTable.getNum(i, 2); genreCheckI[i] = dataTable.getNum(i, 3);
    genreCheckA[i] = dataTable.getNum(i, 4); genreCheckAd[i] = dataTable.getNum(i, 5);
    genreCheckC[i] = dataTable.getNum(i, 6); genreCheckS[i] = dataTable.getNum(i, 7);
    genreCheckR[i] = dataTable.getNum(i, 8); genreCheckSi[i] = dataTable.getNum(i, 9);
    genreCheckE[i] = dataTable.getNum(i, 10); genreCheckSp[i] = dataTable.getNum(i, 11);
    genreCheckRa[i] = dataTable.getNum(i, 12); genreCheckM[i] = dataTable.getNum(i, 13);
  ...
  console.log(genreNf.length);
  console.log(genreIf.length);
  console.log(genreAf.length);
  console.log(genreAdf.length);
  console.log(genreCf.length);
  console.log(genreSf.length);
  console.log(genreRf.length);
  console.log(genreSif.length);
  console.log(genreEf.length);
  console.log(genreSpf.length);
  console.log(genreRaf.length);
  console.log(genreMf.length);
  console.log(genreNp.length);
  console.log(genreIp.length);
  console.log(genreAp.length);
  console.log(genreAdp.length);
  console.log(genreCp.length);
  console.log(genreSp.length);
  console.log(genreRp.length);
  console.log(genreSip.length);
  console.log(genreEp.length);
  console.log(genreSpp.length);
  console.log(genreRap.length);
  console.log(genreMp.length);
...
function ftPDraw() {
    ...
    fill(360);
    textSize(20);
    text("Press 'c' to view", x/4, 650);
    text("FTP", x/4, 350)
    image(img, x/1.5, 300, 300, 300);
    text("Press 'v' to view", x/1.5, 650);
...
function graphF(dataTable, index) {
  ...
  for (var j = 0; j < genreNf.length/2; j++) {
    fill(102,192,244);
    ellipse(this.x * (j+1), this.y, 30, 30);
    text(j, 100, 100);
  ...
  for (var j = 0; j < genreIf.legnth/2; j++) {
    fill(102,192,244);
    ellipse(this.x * (j+1), this.y, 30, 30);
  ...
  for (var j = 0; j < genreAf.length/2; j++) {
    fill(199, 213, 224);
    ellipse(this.x * (j+1), this.y, 30, 30);
